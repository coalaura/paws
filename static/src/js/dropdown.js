...
class Dropdown {
	...
	constructor(el, favorites = false, tabs = []) {
		...
		this.#_select.querySelectorAll("option").forEach(option => {
			const classes = option.dataset.classes?.trim(),
				...
				prices = option.dataset.prices?.trim(),
	...
	#build() {
		...
		for (const option of this.#options) {
			...
			const _titleWrap = make("div", "title-wrap");

			_label.appendChild(_titleWrap);
			...
			// prices
			if (option.prices) {
				const _prices = make("span", "prices");

				_prices.innerHTML = option.prices;

				_label.appendChild(_prices);
			}
